...
import { nameof, nameofFn } from '../nameof';
...
export class Query<T extends EntityBase, R extends T | T[], P = T>
  ...
  private readonly _queryParts: Array<IQueryBuilderPart<T>>;
  ...
  public get queryParts(): Array<IQueryBuilderPart<T>> {
  ...
  public catch(
    rejected: (error: any) => void | Promise<any> | IQuery<any, any>,
  ): Promise<any> {
  ...
  public equalJoined(
    ...
    return this.completeJoinedWhere(
      SqlConstants.OPERATOR_EQUAL,
      selector,
      options,
    );
  ...
  public include<S>(
    ...
    return this.includePropertyUsingAlias<S>(
      propertySelector,
      this._initialAlias,
    );
  }
  ...
  public inSelected<TI extends { id: number }, RI extends TI | TI[], PI1 = TI>(
    ...
    return this.includeOrExcludeFromInnerQuery(
      innerQuery as ISelectQueryInternal<TI, RI, PI1>,
      SqlConstants.OPERATOR_IN,
    );
  }
  ...
  public isolatedAnd<S extends Object>(
    and: (query: IQuery<T, R, P>) => IQuery<T, R, S>,
  ): IQuery<T, R, P> {
    // TODO: These types are not lining up.
    return (
      ((
        this.isolatedConditions<T, P>(
          (and as any) as () => IQuery<T, R, P>,
          this._query.andWhere,
        )
      ) as any)
    ) as IQuery<T, R, P>;
  ...
  public isolatedOr<S extends Object>(
    and: (query: IQuery<T, R, P>) => IQuery<T, R, S>,
  ): IQuery<T, R, P> {
    // TODO: These types are not lining up.
    return (
  ...
  public isolatedWhere<S extends Object>(
    ...
    // TODO: These types are not lining up.
    return this.isolatedConditions<T, S>(
      (where as any) as () => IQuery<T, R, S>,
      this._query.where,
    );
  }
  ...
  public joinAlso<S extends Object>(
    ...
    return this.joinPropertyUsingAlias(
      propertySelector,
      this._initialAlias,
  ...
  public notInSelected<
    TI extends { id: number },
    RI extends TI | TI[],
    PI1 = TI
  >(innerQuery: ISelectQuery<TI, RI, PI1>): IQuery<T, R, P> {
    return this.includeOrExcludeFromInnerQuery(
      innerQuery as ISelectQueryInternal<TI, RI, PI1>,
      SqlConstants.OPERATOR_NOT_IN,
    );
  ...
  public or<S extends Object>(
    propertySelector: (obj: P) => S,
  ): IComparableQuery<T, R, P> {
    return this.andOr(
      propertySelector,
      SqlConstants.OPERATOR_OR,
      this._query.orWhere,
    );
  }
  ...
  public orderBy(propertySelector: (obj: P) => any): IQuery<T, R, P> {
    ...
    this._queryParts.push(
      new QueryBuilderPart(this._query.orderBy, [orderProperty, 'ASC']),
    );
  ...
  public orderByDescending(propertySelector: (obj: P) => any): IQuery<T, R, P> {
    ...
    const orderProperty: string = `${this._lastAlias}.${propertyName}`;
    this._queryParts.push(
      new QueryBuilderPart(this._query.orderBy, [orderProperty, 'DESC']),
    );
  ...
  public reset(): IQuery<T, R, T> {
    ...
    return (this as any) as IQuery<T, R, T>;
  }

  public select(
    propertySelector: (obj: any) => any,
  ): ISelectQuery<T, R, T> | ISelectQuery<T, R, P> {
    const selectedProperties: string[] = nameofFn(propertySelector);
    ...
    this._selectedProperties = selectedProperties.map(
      selectedProperty => `${alias}.${selectedProperty}`,
    );
    this._queryParts.unshift(
      new QueryBuilderPart(this._query.select, [this._selectedProperties]),
    );
  ...
  public skip(skip: number): IQuery<T, R, P> {
    if (skip > 0) {
      this._queryParts.push(new QueryBuilderPart(this._query.skip, [skip]));
    }
  ...
  public take(limit: number): IQuery<T, R, P> {
    if (limit > 0) {
      this._queryParts.push(new QueryBuilderPart(this._query.take, [limit]));
    }
  ...
  public thenBy(propertySelector: (obj: P) => any): IQuery<T, R, P> {
    ...
    const orderProperty: string = `${this._lastAlias}.${propertyName}`;
    this._queryParts.push(
      new QueryBuilderPart(this._query.addOrderBy, [orderProperty, 'ASC']),
    );
  ...
  public thenByDescending(propertySelector: (obj: P) => any): IQuery<T, R, P> {
    ...
    const orderProperty: string = `${this._lastAlias}.${propertyName}`;
    this._queryParts.push(
      new QueryBuilderPart(this._query.addOrderBy, [orderProperty, 'DESC']),
    );
  ...
  public toPromise(): Promise<R> {
    ...
    return this._getAction.call(targetQueryBuilder);
  }
  ...
  public where<S extends Object, F = T | P>(
    ...
    if (this._queryMode === QueryMode.Get) {
      ...
      this._queryParts.push(new QueryBuilderPart(this._query.where, [where]));
    } else {
      // If accessing multiple properties, join relationships using an INNER JOIN.
      whereProperty = this.joinMultipleProperties(whereProperties);
    ...
    return (this as any) as IComparableQuery<T, R, T>;
  }
  ...
  private addJoinCondition(
    ...
    const part: IQueryBuilderPart<T> =
      targetQueryPart || this._queryParts.pop();

    // "includedProperty.property = 'something'"
    let joinCondition: string = (part.queryParams as [string]).pop();
    joinCondition += ` ${condition} ${this._lastAlias}.${whereProperty}`;
    (part.queryParams as [string]).push(joinCondition);
  ...
  private andOr<S extends Object>(
    ...
    const joinAction: (...params: any[]) => SelectQueryBuilder<T> =
      operation === 'AND' ? this._query.innerJoin : this._query.leftJoin;

    // Keep up with the last alias in order to restore it after joinMultipleProperties.
    const lastAlias: string = this._lastAlias;

    // If accessing multiple properties, join relationships using an INNER JOIN.
    const whereProperty: string = this.joinMultipleProperties(
      whereProperties,
      joinAction,
    );
    ...
    if (
      ...
      this._queryParts.push(new QueryBuilderPart(queryAction, [where]));
    }
  ...
  private compileQueryParts<PT>(
    queryParts: Array<IQueryBuilderPart<PT>>,
    builder: WhereExpression,
  ): void {
  ...
  private completeWhere(
    ...
    const nonWheres: Array<IQueryBuilderPart<T>> = [];
    ...
    if (this._queryWhereType === QueryWhereType.Joined) {
      ...
      const part: IQueryBuilderPart<T> = wherePart;
      // "alias.property"
      let where: string = (part.queryParams as [string]).pop();
      ...
      (part.queryParams as [string, ObjectLiteral]).push(where);
      this._queryParts.push(part);
    }
  ...
  private createJoinCondition(joinConditionProperty: string): void {
    ...
    const otherParts: Array<IQueryBuilderPart<T>> = [];
    ...
    while (!targetQueryPart && this._queryParts.length) {
      ...
      if (
        part.queryParams &&
        part.queryParams.length > 1 &&
        part.queryParams[1] === this._lastAlias
      ) {
    ...
    if (targetQueryPart.queryParams.length === 3) {
      ...
      (targetQueryPart.queryParams as [string]).push(joinCondition);
    }
  ...
  >(
    ...
    const query: string = this.buildQuery(innerQuery as any).getQuery();
  ...
  private isolatedConditions<IP, IS>(
    ...
    const query: Query<T, R, IS> = (
      conditions((
        new Query(this._query, this._getAction, this._includeAliasHistory)
      ) as IQuery<T, R, P>)
    ) as Query<T, R, IS>;

    // Do not include joins in bracketed condition; perform those in the outer query.
    const conditionParts: Array<IQueryBuilderPart<T>> = query.queryParts.filter(
      qp =>
        // tslint:disable-next-line: triple-equals
    ...
    const joinParts: Array<IQueryBuilderPart<T>> = query.queryParts.filter(
      qp => conditionParts.indexOf(qp) < 0,
    );
    this._queryParts.push(...joinParts);

    this._queryParts.push(
      new QueryBuilderPart(conditionAction, [
        new Brackets(qb => {
  ...
  private joinForeignEntity<F extends Object>(foreignEntity: new (...params: any[]) => F): IQuery<T, R, F> | IComparableQuery<T, R, F> {
    ...
    if (!this._includeAliasHistory.find(a => a === resultAlias)) {
      ...
      this._queryParts.push(
        new QueryBuilderPart(this._query.innerJoin, [
          foreignEntity,
          resultAlias,
          'true',
        ]),
    ...
    return (this as any) as IQuery<T, R, F> | IComparableQuery<T, R, F>;
  }

  private joinMultipleProperties(
    whereProperties: string,
    joinAction: (...params: any[]) => SelectQueryBuilder<T> = this._query
      .innerJoin,
  ): string {
  ...
  private joinOrIncludePropertyUsingAlias<S extends Object>(
    ...
    if (!this._includeAliasHistory.find(a => a === resultAlias)) {
      ...
      this._queryParts.push(
        new QueryBuilderPart(queryAction, [queryProperty, resultAlias]),
      );
    }

    return (this as any) as IQuery<T, R, S>;
  }

  private joinPropertyUsingAlias<S extends Object>(
    propertySelector: ((obj: T | P) => JoinedEntityType<S>) | string,
    queryAlias: string,
    queryAction: (...params: any[]) => SelectQueryBuilder<T> = this._query
      .innerJoin,
  ): IQuery<T, R, S> {
    return this.joinOrIncludePropertyUsingAlias(
      propertySelector,
