...
import { camelCase, forEach, has, isArray, isEmpty, map, mapValues, omitBy, snakeCase } from 'lodash';
...
export class SwaggerExplorer {
  ...
  public getModelsDefinitons(convertModelObjectKeysCase?: 'snake_case' | 'camelCase') {
    let modelsDefinitions = this.modelsDefinitions;
    if (convertModelObjectKeysCase) {
      modelsDefinitions = map(modelsDefinitions, modelDefinitions => this.transformModelsDefinitionsObjectKeysCase(modelDefinitions, convertModelObjectKeysCase));
    }
    return modelsDefinitions;
  }

  private transformModelsDefinitionsObjectKeysCase(modelDefinitions, convertModelObjectKeysCase?: 'snake_case' | 'camelCase', modelDefinitionsResult = {}) {
    const converterCaseFn = convertModelObjectKeysCase === 'snake_case' ? snakeCase : camelCase;

    forEach(modelDefinitions, (modelDefinition, modelDefinitionName) => {
      const modelDefinitionResult: any = {};
      if (has(modelDefinition, 'properties')) {
        modelDefinitionResult.properties = {};

        forEach(modelDefinition.properties, (modelDefinitionPropertyValue, modelDefinitionPropertyKey) => {
          modelDefinitionPropertyKey = converterCaseFn(modelDefinitionPropertyKey);
          if (has(modelDefinitionPropertyValue, 'allOf')) {
            forEach(modelDefinitionPropertyValue.allOf, (modelDefinitionValueItem, modelDefinitionValueItemKey) => {
              if (has(modelDefinitionValueItem, 'name')) {
                modelDefinitionValueItem.name = converterCaseFn(modelDefinitionValueItem.name);
                modelDefinitionPropertyValue[modelDefinitionValueItemKey] = modelDefinitionValueItem;
              }
            });
          }
          modelDefinitionResult.properties[modelDefinitionPropertyKey] = modelDefinitionPropertyValue;
        });
      }

      if (has(modelDefinition, 'required') && isArray(modelDefinition.required)) {
        modelDefinitionResult.required = modelDefinition.required.map(propertyName => converterCaseFn(propertyName));
      }

      modelDefinitionsResult[modelDefinitionName] = {...modelDefinition, ...modelDefinitionResult};
    });

    return modelDefinitionsResult;
  }
  ...
  private exploreRoutePathAndMethod(_instance, _prototype, method, globalPath) {
