...
import { ResponseSerializerOptions } from '../../../decorators/response-serializer-options.decorator';
...
export interface ResponseMetadata {
  description?: string;
  type?: any;
  typeSerialize?: boolean;
  typeSerializeObjectKeysCase?: 'snake_case' | 'camelCase';
  format?: string;
  isArray?: boolean;
}
...
export const ApiResponse = (
  ...
  const groupedMetadata = { [metadata.status]: omit(metadata, ['status', 'typeSerialize']) };
  return (target, key?, descriptor?: PropertyDescriptor) => {
    if (type && isFunction(type) && metadata.typeSerialize !== false) {
      const responseTypeSerializeDecorator = ResponseSerializerOptions({
        type: metadata.type,
        convertObjectKeysCase: metadata.typeSerializeObjectKeysCase,
      });
