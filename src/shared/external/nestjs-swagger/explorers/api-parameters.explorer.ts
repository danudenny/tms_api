  ...
  return flatMap(reflectedParameters, (param: any) => {
    ...
    return modelProperties.map(key => {
      const reflectedParam =
        ...
        {};
      const propertyName = reflectedParam.name || key;
      return {
        ...param,
        ...reflectedParam,
        name: propertyName,
  ...
  const propertiesWithType = modelProperties.map(key => {
    ...
    if (isNotDefaultType) {
      ...
      if (Object.keys(strippedMetadata).length === 0) {
        return { name: propertyName, required: metadata.required, $ref };
      }
      return {
        name: propertyName,
...
const getEnumType = (values: Array<string | number>): 'string' | 'number' => {
