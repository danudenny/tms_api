...
export class EmployeePenaltyService {
  ...
  static async exportEmployeePenalty (payload: BaseMetaPayloadVm, response){
    try {
      ...
      // q.selectRaw(
      //   ['employee_penalty.employee_penalty_id', 'employeePenaltyId'],
      //   ['TO_CHAR(employee_penalty.penalty_date_time, \'DD Mon YYYY HH24:MI\')', 'penaltyDateTime'],
      //   ['penalty_category.penalty_category_title', 'penaltyCategoryTitle'],
      //   ['representative.representative_code', 'representativeCode'],
      //   ['branch.branch_name', 'branchName'],
      //   ['branch.branch_code', 'branchCode'],
      ...
      q.selectRaw(
        ['TO_CHAR(employee_penalty.penalty_date_time, \'YYYY-MM-DD\') as "Tanggal"', 'penaltyDateTime'],
        ['penalty_category.penalty_category_title as "Kategori"', 'penaltyCategoryTitle'],
        ['representative.representative_code as "Perwakilan"', 'representativeCode'],
        ['CONCAT(branch.branch_code,\' - \',branch.branch_name) as "Cabang"', 'branchCode'],
        ['CONCAT(users.username,\' - \', users.first_name) as "Dibebankan"', 'userName'],
        ['employee_role.employee_role_name as "Jabatan"', 'employeeRoleName'],
        ['employee_penalty.ref_awb_number as "No Resi"', 'refAwbNumber'],
        ['employee_penalty.ref_spk_code as "No SPK/ SJ"', 'refSpkCode'],
        ['employee_penalty.penalty_qty as "Qty"', 'qty'],
        ['employee_penalty.penalty_fee as "Denda"', 'penaltyFee'],
        ['employee_penalty.total_penalty as "Total Denda"', 'totalPenalty'],
        ['employee_penalty.penalty_desc as "Keterangan"', 'penaltyDesc'],
        ['CONCAT(usercreated.username,\' - \',usercreated.first_name) as "User Created"', 'createdUserName'],
        ['CONCAT(usersupdated.username,\' - \',usersupdated.first_name) as "User Updated"', 'updatedUserName'],
        ['employee_penalty.penalty_note as "Sanksi"', 'penaltyNote'],
      );
