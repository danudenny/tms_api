...
@Injectable()
export class SmdScanoutReportService {
  static async exportCSV(
    ...
    await CsvHelper.generateCSV(res, data.data, fileName);
  }
  ...
  public static async retrieveData(id: string): Promise<BaseMetaPayloadVm> {
    const data = await CsvHelper.retrieveGenericData<any>(
      'scan-out-smd',
      id,
    );
  ...
  static async storeExcelPayload(payload: any) {
    const result = {
      id: null,
    };
  ...
  static async storeExcelVendorPayload(payload: any) {
    const result = {
      id: null,
    };
