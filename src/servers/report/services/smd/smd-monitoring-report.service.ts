...
export class SmdMonitoringReportService {
  ...
  public static async retrieveData(id: string): Promise<MonitoringPayloadVm> {
    const data = await CsvHelper.retrieveGenericData<MonitoringPayloadVm>(
      'monitoring-smd',
      id,
    );
    if (!data) {
