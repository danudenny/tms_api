...
import { BaseMetaPayloadVm, BaseMetaPayloadFilterVm, BaseMetaPayloadFilterVmOperator } from '../../../../shared/models/base-meta-payload.vm';
...
export class HubMonitoringService {
  ...
  static async orionFilterToQueryRawBySelectedFilter2(
    filters: BaseMetaPayloadFilterVm[],
    field: string,
    operator: string[],
    key: string[],
    ...
    for (const element of key) {
      const temp1 = await this.orionFilterToQueryRawBySelectedFilter(filters, field, operator, element);
      if (temp1) {
        temp.push(temp1);
      }
    }
